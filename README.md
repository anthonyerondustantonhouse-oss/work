...
1. **Playwright reads the conversation list.** `mvsync login` opens a visible browser; sign in once, the profile is saved. Then `mvsync probe` saves the live page HTML, a screenshot and every conversation-looking link to `./probe/`. Metaview's markup is not documented, so tune `metaview_bullhorn/selectors.py` (or a JSON file named by `METAVIEW_SELECTORS_FILE`) until `mvsync read` prints every conversation with title, date, attendees and a body. A field that cannot be found fails loudly with its name; the reader never returns an empty string for a required field.
